...
export class AdminService {
    ...
    async getStats() {
        ...
        const totalLikes = await this.preferenceRepository.count({ where: { value: 'LIKE' } as any });
        ...
        const matchesQuery = await this.preferenceRepository.query(`
    ...
    async getTopNames(limit = 20) {
        // Get top liked names
        const topNames = await this.preferenceRepository.createQueryBuilder('pref')
        ...
        for (const item of topNames) {
            const name = await this.nameRepository.findOne({ where: { id: item.nameId } });
            if (name) {
                results.push({
                    ...name,
